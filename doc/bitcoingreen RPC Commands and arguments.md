# bitcoingreen RPC Commands and arguments
...
The configuration file is not automatically created; you can create it using your favorite plain-text editor. By default, bitcoingreen-qt (or bitcoingreend) will look for a file named bitcoingreen.conf in the bitcoingreen data directory, but both the data directory and the configuration file path may be changed using the -datadir and -conf command-line arguments.

| Platform	   |Path to data folder           | Typical path to configuration file                                                 |
|------------|--------------------------------|------------------------------------------------------------------------------------|
| Linux	     | ~/	                            | /home/username/.bitcoingreen/bitcoingreen.conf                                               |
| macOS      | ~/Library/Application Support/	| /Users/username/Library/Application Support/Eletra/bitcoingreen.conf                    |
| Windows	   | %APPDATA%	                    | (Vista-10) C:\Users\username\AppData\Roaming\bitcoingreen\bitcoingreen.conf                  |
|            |                                | (2000-XP) C:\Documents and Settings\username\Application Data\bitcoingreen\bitcoingreen.conf |

Note: if running bitcoingreen in testnet mode, the sub-folder testnet will be appended to the data directory automatically. Command line arguments

## These commands are accurate as of bitcoingreen Core version 2.0.0:
...
Start bitcoingreen Core Daemon with option:
<table>
<tr> <td>bitcoingreend</td><td>[options]</td></tr>
</table>
...
<tr> <td>--conf=<file></td><td>Specify configuration file (default: bitcoingreen.conf)</td></tr>
...
<tr> <td>--pid=<file></td><td>Specify pid file (default: bitcoingreend.pid)</td></tr>
...
<tr> <td>--reindexmoneysupply</td><td>Reindex the bitcoingreen and zECA money supply statistics on startup</td></tr>
...
<tr> <td>--paytxfee=<amt></td><td>Fee (in bitcoingreen/kB) to add to transactions you send (default: 0.00)
...
<tr> <td>--minrelaytxfee=<amt></td><td>Fees (in bitcoingreen/Kb) smaller than this are considered zero fee for relaying (default: 0.0001)
...
<tr> <td>-litemode=<n></td><td>Disable all bitcoingreen specific functionality (Masternodes, Zerocoin, wiftX, Budgeting) (0-1, default: 0)
</table>
...
<tr> <td>--ecastake=<n></td><td>Enable or disable staking functionality for bitcoingreen inputs (0-1, default: 1)
...
#### debug ( 0 | 1 | addrman | alert | bench | coindb | db | lock | rand | rpc | selectcoins | mempool | mempoolrej | net | proxy | prune | http | libevent | bitcoingreen )
...
#### preparebudget "proposal-name" "url" payment-count block-start "bitcoingreen-address" monthy-payment
...
#### submitbudget "proposal-name" "url" payment-count block-start "bitcoingreen-address" monthy-payment "fee-tx"
...
PoW Only: Return if the server is set to generate coins or not. The default is false. It is set with the command line argument -gen (or bitcoingreen.conf setting gen) It can also be set with the setgenerate call.
...
#### validateaddress “bitcoingreenaddress”

Return information about the given bitcoingreen address.
...
Wallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same bitcoingreen address.

#### backupwallet "destination"

Safely copies wallet.dat to destination, which can be a directory or a path with filename.

#### bip38decrypt "bitcoingreenaddress" "passphrase"

Decrypts and then imports password protected private key.

#### bip38encrypt "bitcoingreenaddress" "passphrase"

Encrypts a private key corresponding to 'bitcoingreenaddress'.

#### dumpprivkey “bitcoingreenaddress”

Reveals the private key corresponding to ‘bitcoingreenaddress’. Then the importprivkey can be used with this output
...
Returns a new bitcoingreen address for receiving payments. If ‘account’ is specified (DEPRECATED), it is added to the address book so payments received with the address will be credited to ‘account’.

#### getrawchangeaddress

Returns a new bitcoingreen address, for receiving change. This is for use with raw transactions, NOT normal use.

#### getreceivedbyaccount “account” ( minconf addlockconf )

DEPRECATED. Returns the total amount received by addresses with <account> in transactions with specified minimum number of confirmations.

#### getreceivedbyaddress “bitcoingreenaddress” ( minconf addlockconf )

Returns the total amount received by the given bitcoingreenaddress in transactions with specified minimum number of confirmations.
...
#### importprivkey “bitcoingreenprivkey” ( “label” rescan )
...
#### sendfrom “fromaccount” “tobitcoingreenaddress” amount ( minconf addlockconf “comment” “comment-to” )
...
#### sendtoaddress “bitcoingreenaddress” amount ( “comment” “comment-to” subtractfeefromamount use_is use_ps )
...
Stores the wallet decryption key in memory for ‘timeout’ seconds. This is needed prior to performing transactions related to private keys such as sending bitcoingreens
...
Usage:
  bitcoingreen-tx [options] <hex-tx> [commands]  Update hex-encoded bitcoingreen transaction
  bitcoingreen-tx [options] -create [commands]   Create hex-encoded bitcoingreen transaction
